...
<pre>
	1 git merge is that git merge is a way of combining changes from one branch into another, 
	preserving the history of the source branch
	1 git rebase is a way of moving the changes from one branch onto another, typically the master branch,
	and flattening the history by removing unwanted entries.

	2 When using git merge, a new merge commit is created
	2 git rebase creates a new set of commits applied on top of the target branch, 
	resulting in a linear history.

	3 git merge is used to join two or more development histories together, 
	preserving the original history
	3 git rebase is used to integrate the changes from one branch into another, creating a linear history 
	and applying changes in a more orderly manner

	4 Functionality: git merge creates a new commit that combines the changes from one branch (the source branch) into another (the destination branch).
	...
	4 Functionality: git rebase moves or rewrites the commit history of a branch by applying the commits from one branch onto another branch.
	...
	Clarity in History: It produces a cleaner and more linear history, as if the changes on the rebased branch were originally made on top of the destination branch. However, 
	it rewrites commit history, which could potentially cause conflicts or difficulties for collaborators who also have the old commits.
</pre>
</body>
</html>
